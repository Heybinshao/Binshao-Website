...
export function SiteHeader() {
  ...
  const [themeMode, setThemeMode] = useState<"sunny" | "light" | "dark">("light")

  // 从 head 脚本同步的 DOM class 读取实际主题
  useEffect(() => {
    const h = document.documentElement
    if (h.classList.contains("sunny")) setThemeMode("sunny")
  ...
  function applyTheme(mode: "sunny" | "light" | "dark") {
    ...
    html.classList.remove("sunny", "light", "dark")
    ...
    if (meta) {
      meta.setAttribute(
        "content",
        mode === "dark" ? META_THEME_COLORS.dark
        : mode === "sunny" ? META_THEME_COLORS.sunny
        : META_THEME_COLORS.light
      )
    }
  }

  const cycleModes: ("sunny" | "light" | "dark")[] = ["sunny", "light", "dark"]
  const cycleTheme = useCallback(() => {
    const idx = cycleModes.indexOf(themeMode)
    const nextMode = cycleModes[(idx + 1) % cycleModes.length]
    click()
